...
class VLAConsumerDataset(Dataset):
    ...
    def __init__(
        ...
        # 🆕 解析关键时间段标注配置
        if critical_annotation_config is None:
            critical_annotation_config = {}
        
        # 从配置中提取随机标注参数
        use_random_annotation = critical_annotation_config.get('use_random_annotation', False)
        random_critical_ratio = critical_annotation_config.get('random_critical_ratio', 0.3)
        random_seed = critical_annotation_config.get('random_seed', None)
        # 🔍 调试输出
        print("🔍 调试信息 - 标注器配置:")
        print(f"   critical_annotation_config 原始内容: {critical_annotation_config}")
        print(f"   use_random_annotation: {use_random_annotation} (类型: {type(use_random_annotation)})")
        print(f"   random_critical_ratio: {random_critical_ratio}")
        print(f"   random_seed: {random_seed}")
        print(f"   task_type: {task_type}")
        # 创建标注器
        if self.enable_critical_annotation:
            
            self.annotator = create_silent_task_annotator(
                task_type=TaskType(task_type),
                use_random=use_random_annotation,      # 🆕 传递随机模式
                random_ratio=random_critical_ratio,    # 🆕 传递随机比例
                random_seed=random_seed              # 🆕 传递随机种子
            )
            # 🆕 打印标注器详细信息
            print(f"🎯 关键时间段标注器初始化成功")
            print(f"   📋 标注模式: {'🎲 随机标注' if use_random_annotation else '🎯 任务驱动'}")
            if use_random_annotation:
                print(f"   📊 关键帧比例: {random_critical_ratio:.1%} (目标)")
                print(f"   🔒 随机种子: {random_seed if random_seed is not None else '未设置'}")
            else:
                task_name = "抓取类任务" if task_type == 1 else "点击类任务"
                print(f"   🎯 任务类型: {task_name} (TaskType.{TaskType(task_type).name})")
            print(f"   ✅ 标注器对象: {type(self.annotator).__name__}")
        else:
            self.annotator = None
            print("⚠️  关键时间段标注已禁用")
    ...
    def _generate_critical_labels(self, qpos_trajectory, action_horizon=64):
        ...
        try:
            ...
            # 使用标注器生成标签
            critical_labels, analysis_info = self.annotator.annotate(qpos_trajectory)
            ...
            try:
                ...
                if self.enable_critical_annotation:
                    try:
                        ...
                        # 可选：记录标注统计信息（用于调试）
                        if hasattr(self.annotator, 'verbose') and self.annotator.verbose:
